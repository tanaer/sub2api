<template>
  <AppLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-name="{ row, value }">
            <div class="flex flex-col">
              <div class="flex items-center gap-1.5">
                <span class="text-xs font-mono text-gray-400 dark:text-gray-500">#{{ row.id }}</span>
                <span class="font-medium text-gray-900 dark:text-white">{{ value }}</span>
                <span
                  v-if="row.priority != null"
                  class="inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-medium leading-none bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400"
                  :title="t('admin.accounts.columns.priority')"
                >P{{ row.priority }}</span>
              </div>
              <span
                v-if="row.upstream_provider"
                class="text-[11px] text-gray-400 dark:text-gray-500"
              >{{ row.upstream_provider }}</span>
              <span
          ...
          <template #cell-health_score="{ row }">
            <span
              v-if="typeof row.health_score === 'number'"
              class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium"
              :class="row.health_score >= 90 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300'
                : row.health_score >= 70 ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300'
                : row.health_score >= 50 ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300'
                : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300'"
              :title="`${t('admin.accounts.columns.healthScore')}: ${row.health_score}%`"
            >
              {{ row.health_score }}%
            </span>
    ...
    <AccountActionMenu :show="menu.show" :account="menu.acc" :position="menu.pos" @close="menu.show = false" @copy="handleCopy" @test="handleTest" @stats="handleViewStats" @schedule="handleSchedule" @reauth="handleReAuth" @refresh-token="handleRefresh" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" />
...
const allColumns = computed(() => {
  const c = [
    ...
    { key: 'health_score', label: t('admin.accounts.columns.healthScore'), sortable: true },
...
const mergeRuntimeFields = (oldAccount: Account, updatedAccount: Account): Account => ({
  ...
  health_score: updatedAccount.health_score ?? oldAccount.health_score,
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `aiapi-account-${timestamp}.json`
