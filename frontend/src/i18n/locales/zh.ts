export default {
  ...
  dashboard: {
    ...
    cacheToday: '今日缓存',
    groupRequestQuota: '分组按次配额',
    groupRequestQuotaHint: '查看每个分组的已用次数和剩余次数',
    permanentQuota: '永久额度',
    grantUnit: '次',
    quotaBreakdown: '额度明细',
    expiresAt: '{time} 到期',
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      openclaw: {
        description: '',
        importDescription: '导入本地配置后，系统会更新当前 provider、模型列表和默认模型。',
        defaultPathLabel: '默认路径',
        rememberHint: '支持的浏览器会记住上次选择的目录',
        selectFile: '选择 openclaw.json / openclaw.json5',
        providerLabel: 'Provider',
        modelLabel: '默认模型',
        resultHint: '下载或复制下面的内容后，覆盖本地 OpenClaw 配置文件即可。',
        note: '',
        parseFailed: '无法解析 OpenClaw 配置文件，请确认文件内容是有效的 JSON5 / JSON。',
        generateFailed: '生成 OpenClaw 配置失败，请稍后重试。',
        unsupported: '当前分组类型暂不支持生成 OpenClaw 配置。'
      }
    },
  ...
  redeem: {
    title: '兑换码',
    description: '输入兑换码以充值余额、增加并发数或兑换分组次数',
    currentBalance: '当前余额',
    concurrency: '并发数',
    requests: '请求',
    redeemCodeLabel: '兑换码',
    ...
    codeRule2: '兑换码可以增加余额、并发数、分组次数或试用权限',
    codeRule3: '如有兑换问题，请联系客服',
    codeRule4: '余额、并发数和分组次数即时更新',
    recentActivity: '最近活动',
    ...
    groupRequestQuotaAddedRedeem: '分组次数增加（兑换）',
    groupRequestQuotaAdded: '分组次数已增加',
    groupRequestQuotaAddedDesc: '您已获得 {groupName} 分组 {count} 次可用次数',
    groupRequestQuotaExpiresAt: '该批次数额度将于 {time} 失效',
    groupRequestQuotaExpiresAtShort: '失效时间：{time}',
  ...
  admin: {
    ...
    backup: {
      ...
      r2Guide: {
        ...
        step1: {
          ...
          line2: '点击「创建存储桶」，输入名称（如 aiapi-backups），选择区域',
          line3: '点击创建完成'
        },
        step2: {
          title: '创建 API 令牌',
    ...
    users: {
      ...
      none: '无',
      requestQuota: '按次配额',
      requestQuotaHint: '设置为 0 表示关闭按次计费',
      saveRequestQuota: '保存次数',
      resetRequestQuotaUsed: '重置已用',
      requestQuotaUsed: '已使用次数',
      requestQuotaRemaining: '剩余次数',
      requestQuotaSaved: '按次配额已更新',
      requestQuotaUsedReset: '已使用次数已重置',
      requestQuotaSaveFailed: '按次配额更新失败',
      requestQuotaInvalid: '请输入大于等于 0 的整数次数',
      ...
      // 用户分组配置
      groupConfig: '用户分组配置',
      groupConfigHint: '为用户 {email} 配置专属分组倍率和按次配额（覆盖分组默认倍率）',
    ...
    groups: {
      ...
      useKeyInstructionsPlaceholder: '可选，填写后会显示在用户侧“使用密钥”弹层顶部，并覆盖默认说明文案',
      useKeyInstructionsHint: '按分组自定义 API 密钥使用说明，适合放接入提示、注意事项或专属配置要求。',
    ...
    accounts: {
      ...
      types: {
        ...
        soraApiKeyHint: '连接另一个 AIAPI 或兼容 API',
      ...
      modelFallbackChain: '模型降级重试链',
      modelFallbackChainHint: '仅在上游 429 / 529 / 限流 / 过载类错误时生效。主模型失败后，按顺序尝试后备模型，最多 3 次。',
      modelFallbackChainExampleTitle: '配置示例',
      modelFallbackChainExample: '请求 gpt-5 时，先按当前映射命中的主模型请求；若主模型因并发/限流/过载失败，再依次尝试 gpt-4.1、gpt-4o。',
      modelFallbackChainNote: '左侧填写用户请求模型；右侧填写失败后的候选模型列表。',
      modelFallbackChainPreviewLabel: '链路预览',
      addModelFallbackChain: '添加降级链',
      modelFallbackTargetsPlaceholder: '失败后依次尝试，例如：gpt-4.1, gpt-4o-mini',
      modelFallbackTargetsHint: '使用逗号分隔后备模型；主模型仍取自当前映射或原始请求模型',
      poolMode: '池模式',
      poolModeHint: '上游为账号池时启用，错误不标记本地账号状态',
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 AIAPI 实例的场景。',
      ...
      soraUpstreamBaseUrlHint: '上游 Sora 服务地址（另一个 AIAPI 实例或兼容 API）',
    ...
    redeem: {
      ...
      validityDays: '有效天数',
      groupRequestQuotaValidityHint: '默认 30 天。从用户兑换成功时开始计算；每次兑换都会单独记一笔次数额度，到期自动失效。',
      ...
      noCodes: '暂无兑换码',
      noCodesDescription: '生成兑换码以向用户分发余额、并发数、订阅或分组次数。',
    ...
    ops: {
      ...
      requestTrace: {
        title: '请求追踪',
        empty: '暂无请求追踪数据',
        failedToLoad: '加载请求追踪失败',
        copied: '标识已复制',
        copyFailed: '复制失败',
        incomplete: '链路未完整结束',
        ambiguousTitle: '该标识命中了多条请求链路',
        ambiguousHint: '请从下面的候选 client_request_id 中选择一条继续查看。',
        useCandidate: '查看这条',
        notFound: '未找到对应的请求追踪',
        notFoundHint: '可以换 client/local/usage/upstream 请求标识再试一次。',
        timelineEmpty: '暂无链路事件',
        timelineCount: '个事件',
        noEventData: '无附加数据',
        boolean: {
          yes: '是',
        ...
        fields: {
          ...
          queryKeyType: '查询类型',
          matchedBy: '命中方式',
          clientRequestId: '客户端请求ID',
          localRequestId: '本地请求ID',
          usageRequestId: '用量请求ID',
          upstreamRequestIds: '上游请求ID',
          originalRequestedModel: '原始请求模型',
          groupResolvedModel: '分组解析模型',
          accountSupportLookupModel: '账号支持匹配模型',
          finalUpstreamModel: '最终上游模型',
          createdAt: '开始时间',
          finishedAt: '结束时间',
          duration: '耗时',
          status: '状态',
          ...
          groupId: '分组ID',
          finalStatus: '最终状态',
          finalStatusCode: '最终状态码',
          finalAccount: '最终账号',
          finalAccountId: '最终账号ID',
          traceIncomplete: '链路完整性'
        }
      },
      alertEvents: {
        title: '告警事件',
    ...
    settings: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 AIAPI 用户登录',
      ...
      modelIdentity: {
        title: '模型身份伪装',
        description: '控制上游模型身份信息的拦截和改写行为',
        localResponseEnabled: '本地拦截',
        localResponseEnabledHint: '检测到用户询问模型身份时，直接返回伪装回复，不发送到上游。',
        instructionInjectionEnabled: '指令注入',
        instructionInjectionEnabledHint: '在请求的 System Prompt 中注入身份回答约束指令。',
        responseRewriteEnabled: '响应改写',
        responseRewriteEnabledHint: '扫描上游响应，检测并替换身份泄露内容。身份问题使用关键词匹配（严格），非身份问题使用句式匹配（宽松）。',
        hitWordsTitle: '关键词列表',
        hitWordsHint: '用于严格模式的身份关键词。当用户询问身份问题时，响应中包含这些词即触发改写。',
        hitWordsPlaceholder: '输入关键词后回车添加',
        identityPatternsTitle: '身份句式（正则）',
        identityPatternsHint: '用于宽松模式的正则表达式（每行一个）。当用户未询问身份问题时，仅匹配这些句式才触发改写。',
        identityPatternsPlaceholder: '每行一个正则表达式',
        replyTemplateTitle: '回复模板',
        replyTemplateHint: '支持 {company} 和 {model} 占位符，会根据请求的模型名自动填充。',
        replyTemplatePlaceholder: '我是一个由{company}训练的{model}大语言模型...',
        resetDefaults: '重置为默认值',
        resetDefaultsConfirm: '确定要重置为默认配置吗？',
      },
      healthBreaker: {
        title: '健康度熔断器',
  ...
  onboarding: {
    ...
    admin: {
      welcome: {
        title: '👋 欢迎使用 AIAPI',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">AIAPI 是一个强大的 AI 服务中转平台，让您轻松管理和分发 AI 服务。</p><p style="margin-bottom: 12px;"><b>🎯 核心功能：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>📦 <b>分组管理</b> - 创建不同的服务套餐（VIP、免费试用等）</li><li>🔗 <b>账号池</b> - 连接多个上游 AI 服务商账号</li><li>🔑 <b>密钥分发</b> - 为用户生成独立的 API Key</li><li>💰 <b>计费管理</b> - 灵活的费率和配额控制</li></ul><p style="color: #10b981; font-weight: 600;">接下来，我们将用 3 分钟带您完成首次配置 →</p></div>',
        nextBtn: '开始配置 🚀',
        prevBtn: '跳过'
      },
      groupManage: {
        title: '📦 第一步：分组管理',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;"><b>什么是分组？</b></p><p style="margin-bottom: 12px;">分组是 AIAPI 的核心概念，它就像一个"服务套餐"：</p><ul style="margin-left: 20px; margin-bottom: 12px; font-size: 13px;"><li>🎯 每个分组可以包含多个上游账号</li><li>💰 每个分组有独立的计费倍率</li><li>👥 可以设置为公开或专属分组</li></ul><p style="margin-top: 12px; padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>💡 示例：</b>您可以创建"VIP专线"（高倍率）和"免费试用"（低倍率）两个分组</p><p style="margin-top: 16px; color: #10b981; font-weight: 600;">👉 点击左侧的"分组管理"开始</p></div>'
      },
      createGroup: {
        title: '➕ 创建新分组',
        description:
    ...
    user: {
      welcome: {
        title: '👋 欢迎使用 AIAPI',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 AIAPI AI 服务平台。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 创建 API 密钥</li><li>📋 复制密钥到您的应用</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: '开始 🚀',
        prevBtn: '跳过'
      },
      keyManage: {
