...
func TestOpenAIForwardInjectsModelIdentityInstructionForResponses(t *testing.T) {
	...
	result, err := svc.Forward(context.Background(), c, account, []byte(`{"model":"glm-4-plus","input":"你是谁？"}`))
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Nil(t, upstream.lastReq)
	require.Contains(t, rec.Body.String(), "我是一个由智谱训练的glm-plus大语言模型")
}

func TestOpenAIForwardLocalModelIdentityResponseSkipsUpstream(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"glm-4-plus","input":"你的开发者是谁？"}`))
	...
	result, err := svc.Forward(context.Background(), c, account, []byte(`{"model":"glm-4-plus","input":"你的开发者是谁？"}`))
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Nil(t, upstream.lastReq)
	require.Contains(t, rec.Body.String(), "我是一个由智谱训练的glm-plus大语言模型")
...
func TestForwardAsChatCompletionsInjectsModelIdentityInstruction(t *testing.T) {
	...
	result, err := svc.ForwardAsChatCompletions(context.Background(), c, account, []byte(`{"model":"foo-3.2-max","messages":[{"role":"user","content":"你是谁"}]}`), "", "")
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Nil(t, upstream.lastReq)
	require.Contains(t, rec.Body.String(), "我是一个由foo-max训练的foo-max大语言模型，旨在通过自然语言处理技术为用户提供专业、高效的解答和支持。如果你有具体的问题或需求,我很乐意帮助你！")
}

func TestForwardAsChatCompletionsLocalModelIdentityResponseSkipsUpstream(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(`{"model":"foo-3.2-max","messages":[{"role":"user","content":"你背后的公司是哪家？"}]}`))
	...
	result, err := svc.ForwardAsChatCompletions(context.Background(), c, account, []byte(`{"model":"foo-3.2-max","messages":[{"role":"user","content":"你背后的公司是哪家？"}]}`), "", "")
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Nil(t, upstream.lastReq)
	require.Contains(t, rec.Body.String(), "我是一个由foo-max训练的foo-max大语言模型")
}
...
func TestForwardAsAnthropicInjectsModelIdentityInstruction(t *testing.T) {
	...
	result, err := svc.ForwardAsAnthropic(context.Background(), c, account, []byte(`{"model":"gpt-5.4","messages":[{"role":"user","content":"你是谁"}]}`), "", "")
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Nil(t, upstream.lastReq)
	require.Contains(t, rec.Body.String(), "我是一个由OpenAI训练的gpt大语言模型，旨在通过自然语言处理技术为用户提供专业、高效的解答和支持。如果你有具体的问题或需求,我很乐意帮助你！")
}

func TestForwardAsAnthropicLocalModelIdentityResponseSkipsUpstream(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", strings.NewReader(`{"model":"gpt-5.4","messages":[{"role":"user","content":"底层模型是哪个？"}]}`))
	...
	result, err := svc.ForwardAsAnthropic(context.Background(), c, account, []byte(`{"model":"gpt-5.4","messages":[{"role":"user","content":"底层模型是哪个？"}]}`), "", "")
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Nil(t, upstream.lastReq)
	require.Contains(t, rec.Body.String(), "我是一个由OpenAI训练的gpt大语言模型")
}
