...
func (c *Client) init() {
	...
	c.AccountThrottleRule = NewAccountThrottleRuleClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		AccountThrottleRule:     NewAccountThrottleRuleClient(cfg),
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.APIKey, c.Account, c.AccountGroup, c.AccountThrottleRule, c.Announcement,
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *AccountThrottleRuleMutation:
		return c.AccountThrottleRule.mutate(ctx, m)
...
// AccountThrottleRuleClient is a client for the AccountThrottleRule schema.
type AccountThrottleRuleClient struct {
	config
}

// NewAccountThrottleRuleClient returns a client for the AccountThrottleRule from the given config.
func NewAccountThrottleRuleClient(c config) *AccountThrottleRuleClient {
	return &AccountThrottleRuleClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `accountthrottlerule.Hooks(f(g(h())))`.
func (c *AccountThrottleRuleClient) Use(hooks ...Hook) {
	c.hooks.AccountThrottleRule = append(c.hooks.AccountThrottleRule, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `accountthrottlerule.Intercept(f(g(h())))`.
func (c *AccountThrottleRuleClient) Intercept(interceptors ...Interceptor) {
	c.inters.AccountThrottleRule = append(c.inters.AccountThrottleRule, interceptors...)
}

// Create returns a builder for creating a AccountThrottleRule entity.
func (c *AccountThrottleRuleClient) Create() *AccountThrottleRuleCreate {
	mutation := newAccountThrottleRuleMutation(c.config, OpCreate)
	return &AccountThrottleRuleCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of AccountThrottleRule entities.
func (c *AccountThrottleRuleClient) CreateBulk(builders ...*AccountThrottleRuleCreate) *AccountThrottleRuleCreateBulk {
	return &AccountThrottleRuleCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *AccountThrottleRuleClient) MapCreateBulk(slice any, setFunc func(*AccountThrottleRuleCreate, int)) *AccountThrottleRuleCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AccountThrottleRuleCreateBulk{err: fmt.Errorf("calling to AccountThrottleRuleClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AccountThrottleRuleCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for AccountThrottleRule.
func (c *AccountThrottleRuleClient) Update() *AccountThrottleRuleUpdate {
	mutation := newAccountThrottleRuleMutation(c.config, OpUpdate)
	return &AccountThrottleRuleUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *AccountThrottleRuleClient) UpdateOne(_m *AccountThrottleRule) *AccountThrottleRuleUpdateOne {
	mutation := newAccountThrottleRuleMutation(c.config, OpUpdateOne, withAccountThrottleRule(_m))
	return &AccountThrottleRuleUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *AccountThrottleRuleClient) UpdateOneID(id int64) *AccountThrottleRuleUpdateOne {
	mutation := newAccountThrottleRuleMutation(c.config, OpUpdateOne, withAccountThrottleRuleID(id))
	return &AccountThrottleRuleUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for AccountThrottleRule.
func (c *AccountThrottleRuleClient) Delete() *AccountThrottleRuleDelete {
	mutation := newAccountThrottleRuleMutation(c.config, OpDelete)
	return &AccountThrottleRuleDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *AccountThrottleRuleClient) DeleteOne(_m *AccountThrottleRule) *AccountThrottleRuleDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *AccountThrottleRuleClient) DeleteOneID(id int64) *AccountThrottleRuleDeleteOne {
	builder := c.Delete().Where(accountthrottlerule.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &AccountThrottleRuleDeleteOne{builder}
}

// Query returns a query builder for AccountThrottleRule.
func (c *AccountThrottleRuleClient) Query() *AccountThrottleRuleQuery {
	return &AccountThrottleRuleQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAccountThrottleRule},
		inters: c.Interceptors(),
	}
}

// Get returns a AccountThrottleRule entity by its id.
func (c *AccountThrottleRuleClient) Get(ctx context.Context, id int64) (*AccountThrottleRule, error) {
	return c.Query().Where(accountthrottlerule.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AccountThrottleRuleClient) GetX(ctx context.Context, id int64) *AccountThrottleRule {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *AccountThrottleRuleClient) Hooks() []Hook {
	return c.hooks.AccountThrottleRule
}

// Interceptors returns the client interceptors.
func (c *AccountThrottleRuleClient) Interceptors() []Interceptor {
	return c.inters.AccountThrottleRule
}

func (c *AccountThrottleRuleClient) mutate(ctx context.Context, m *AccountThrottleRuleMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AccountThrottleRuleCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AccountThrottleRuleUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AccountThrottleRuleUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AccountThrottleRuleDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AccountThrottleRule mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		APIKey, Account, AccountGroup, AccountThrottleRule, Announcement,
