...
// The AccountThrottleRuleFunc type is an adapter to allow the use of ordinary
// function as AccountThrottleRule mutator.
type AccountThrottleRuleFunc func(context.Context, *ent.AccountThrottleRuleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AccountThrottleRuleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AccountThrottleRuleMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AccountThrottleRuleMutation", m)
}
