...
type Group struct {
	...
	// 用户侧使用密钥弹层的自定义说明
	UseKeyInstructions *string `json:"use_key_instructions,omitempty"`
	// 分组配置文件模板（JSON 格式，用于使用密钥弹层显示）
	ConfigTemplates *string `json:"config_templates,omitempty"`
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldModelRouting, group.FieldModelAliases, group.FieldSupportedModelScopes:
			values[i] = new([]byte)
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldUseKeyInstructions:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field use_key_instructions", values[i])
			} else if value.Valid {
				_m.UseKeyInstructions = new(string)
				*_m.UseKeyInstructions = value.String
			}
		case group.FieldConfigTemplates:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field config_templates", values[i])
			} else if value.Valid {
				_m.ConfigTemplates = new(string)
				*_m.ConfigTemplates = value.String
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.UseKeyInstructions; v != nil {
		builder.WriteString("use_key_instructions=")
		builder.WriteString(*v)
	}
	builder.WriteString(", ")
	if v := _m.ConfigTemplates; v != nil {
		builder.WriteString("config_templates=")
		builder.WriteString(*v)
	}
	builder.WriteString(", ")
